...
*impt* is intended to satisfy different types of users: developers, testers, product and factory administrators, etc. Different users may have different requirements for a verbosity level and the format of the output generated by the tool.

The default verbose output is mostly intended for a manual interaction with the tool by developers and testers.

Every command has the `--output <mode>` option (alias: `-z`). It can be used to adjust the default output generated by the command, eg. to minimize the output and/or to present it in a format which is more convenient for processing by scripts. The supported output modes are described in the [Commands Manual](./CommandsManual.md#command-output).
...
If `--output debug` mode is specified, the tool displays debug information from the command being executed, including impCentral API requests and responses.

### Script Support ###

*impt* makes it easy for scripts to process its output. As such, user interaction is minimal. Only a few commands, such as [delete](#entity-deletion), ask for a confirmation from the user. But all of these commands support the `--confirmed` option (alias: `-q`), which, if specified, causes the command to be executed without requiring confirmation from the user. Scripts can use this option.

Scripts can also use one of the methods to understand a [result of the command execution](#command-execution-result) &mdash; an exit code or a predefined phrase.

Additional modes, eg. `--output json`, are also intended to present the generated output in formats more convenient for processing by scripts.
