...
*impt* is intended to satisfy different types of users - developers, testers, product and factory administrators, etc. Different users may have different requirements for a verbosity level and a format of the output generated by the tool.

The default verbose output is mostly intended for a manual interaction with the tool by developers and testers.

Every command has a `--output <mode>` option (alias: `-z`). It can be used to adjust the default output generated by the command. Eg. to minimize the output and/or to have it in a format which is more convenient for processing by scripts. The supported output modes are described in the [Commands Manual](./CommandsManual.md#command-output).

### Command Execution Result ###

Irrespective of the output mode if a command fails, it returns non-zero exit code. Otherwise, it returns zero exit code.
...
If `--output debug` mode is specified, the tool additionally displays debug information from the command being executed, including impCentral API requests and responses.
...
Scripts can use one of the methods to understand a [result of the command execution](#command-execution-result) - an exit code or a predefined phrase.

Additional modes, eg. `--output json`, are intended to present the generated output in formats more convenient for processing by scripts.

### Output Examples ###

The same command in different output modes:
