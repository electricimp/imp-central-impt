...
| --device-file | -x | No | Yes | The device source code file name. If not specified and `--agent-only` option is not specified either, the file referenced by the [Project file](#project-files) in the current directory is used (if there is no Project file, the command fails) |
| --agent-file | -y | No | Yes | The agent source code file name. If not specified and `--device-only` option is not specified either, the file referenced by the [Project file](#project-files) in the current directory is used (if there is no Project file, the command fails) |
